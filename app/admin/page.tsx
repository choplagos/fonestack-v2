...
const adminSchema = {
  ...
  '@type': 'WebPage',
  name: 'Admin Dashboard - Fonestack',
  description: 'Fonestack administrative dashboard for managing inventory and sales.',
}
    ...
    <>
      <Script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(adminSchema),
        }}
      />
      
      <div className="space-y-8 p-4" role="main" aria-label="Admin Dashboard">
        {/* Stat Lens Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6" role="region" aria-label="Dashboard statistics">
          {stats.map((stat, idx) => (
            <motion.div
              key={idx}
              whileHover={{ y: -5 }}
            ...
            >
              {/* Subtle radial glow */}
              <div className={`absolute top-[-20%] right-[-20%] w-32 h-32 blur-3xl opacity-20 rounded-full ${stat.color.replace('text-', 'bg-')}`} aria-hidden="true" />
              
              <stat.icon className={`w-8 h-8 ${stat.color} mb-6`} aria-hidden="true" />
              ...
              <div className="mt-4 text-[10px] font-bold py-1 px-3 bg-white/5 rounded-full w-fit">{stat.trend}</div>
            </motion.div>
          ))}
        </div>

        {/* Recent Activity Glass Table */}
        <div className="liquid-glass rounded-[2.5rem] border-white/5 p-8" role="region" aria-label="Recent transactions">
          <div className="flex justify-between items-center mb-8">
            <h3 className="text-xl font-bold dark:text-white">Recent Transactions</h3>
            <button className="text-xs font-mono text-premiumYellow hover:underline" aria-label="View all sales">View All Sales</button>
          </div>

          <div className="overflow-x-auto">
            <table className="w-full text-left" role="table" aria-label="Transaction history">
              <thead>
                <tr className="border-b border-white/5 text-[10px] font-mono uppercase tracking-widest opacity-40">
                  <th className="pb-4 px-4" scope="col">Invoice</th>
                  <th className="pb-4 px-4" scope="col">Customer</th>
                  <th className="pb-4 px-4" scope="col">Device</th>
                  <th className="pb-4 px-4" scope="col">Amount</th>
                  <th className="pb-4 px-4" scope="col">Status</th>
                </tr>
              </thead>
