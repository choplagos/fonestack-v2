    ...
    >
      ...
      <div className="flex justify-between items-start z-10">
        ...
        <button
          ...
          className={`p-1 sm:p-1.5 md:p-2 rounded-full liquid-glass transition-colors ${isInWishlist ? 'text-premiumYellow bg-premiumYellow/10' : 'text-slate-400'}`}
        >
      ...
      <div className="relative h-20 sm:h-32 md:h-44 w-full flex items-center justify-center p-1 sm:p-2 md:p-4">
        {isSoldOut && (
          <div className="absolute inset-0 flex items-center justify-center z-20">
            <span className="bg-black/80 text-white border border-white/20 px-2 py-1 rounded-full text-[8px] sm:text-[10px] font-bold tracking-widest">SOLD OUT</span>
          </div>
        )}
        {product.image_url ? (
          <img
          ...
          <Smartphone className="w-8 h-8 sm:w-12 sm:h-12 md:w-16 md:h-16 text-slate-300 dark:text-slate-600" />
        )}
      </div>
      ...
      <div className="flex flex-col gap-0.5 sm:gap-1">
        ...
        <h3 className="font-bold text-slate-900 dark:text-white group-hover:text-premiumYellow transition-colors line-clamp-1 text-[10px] sm:text-xs md:text-sm">
          {product.name}
        </h3>
        <p className="text-[9px] sm:text-[10px] md:text-xs text-slate-500 dark:text-slate-400 line-clamp-2 hidden sm:block">
          {product.spec || 'Contact for specs.'}
        </p>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between pt-2 sm:pt-3 md:pt-4 border-t border-white/5">
        <div className="flex flex-col">
          ...
          <span className="text-[10px] sm:text-xs md:text-base font-display font-extrabold text-slate-900 dark:text-white">
