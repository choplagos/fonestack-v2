...
export default function AICompareSection({ phones }: { phones: any[] }) {
  ...
  return (
    <div className="liquid-glass p-8 rounded-[2.5rem] border-premiumYellow/20 shadow-[0_0_50px_rgba(230,255,0,0.05)]">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
        <div>
          <div className="flex items-center gap-2 text-premiumYellow font-mono text-xs tracking-widest uppercase mb-2">
            <Sparkles className="w-4 h-4" /> AI Comparison Engine
          </div>
      ...
      {!result && !isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-8">
          {/* Verdict Text */}
          <div className="text-lg leading-relaxed dark:text-slate-200 font-medium">
            "{result.verdict}"
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-4">
            {result.scores.map((s: any, idx: number) => (
              <div key={idx} className="bg-black/20 p-4 rounded-2xl border border-white/5">
                <div className="text-[10px] uppercase font-mono opacity-50 mb-1">{s.name}</div>
                <div className="text-3xl font-display font-black text-premiumYellow">{s.value_score}%</div>
          ...
          <div className="bg-premiumYellow/10 border border-premiumYellow/20 p-6 rounded-2xl flex items-start gap-4">
             <Trophy className="text-premiumYellow w-8 h-8 shrink-0" />
             <div>
          ...
          <div className="pt-6 border-t border-white/5">
             <div className="flex gap-3">
                ...
                />
                <button className="bg-white/10 p-3 rounded-xl hover:bg-premiumYellow hover:text-black transition-all">
